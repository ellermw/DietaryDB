#!/bin/bash
# /opt/dietarydb/fix-category-display.sh
# Fix category display issue - categories exist but not showing
...
echo "Step 1: Checking database categories..."
echo "======================================="
echo "Categories in database:"
docker exec dietary_postgres psql -U dietary_user -d dietary_db -c "SELECT * FROM categories ORDER BY category_name;" || echo "No categories table"
echo ""
echo "Items by category:"
docker exec dietary_postgres psql -U dietary_user -d dietary_db -c "SELECT category, COUNT(*) FROM items WHERE is_active = true GROUP BY category;" || echo "No items"
echo ""
...
curl -s http://localhost:3000/api/categories | python3 -m json.tool || echo "API not responding"
echo ""

# Step 3: Create a diagnostic endpoint and fix the backend
echo "Step 3: Creating fixed backend with diagnostic endpoint..."
echo "=========================================================="

cat > backend/server-display-fix.js << 'EOF'
...
# Deploy backend
docker cp backend/server-display-fix.js dietary_backend:/app/server.js
echo "Backend updated with display fix"
echo ""
...
echo "Step 5: Testing fixed API..."
echo "============================"
echo "Categories API response after fix:"
curl -s http://localhost:3000/api/categories | python3 -m json.tool || echo "API error"
echo ""

echo "Debug endpoint:"
curl -s http://localhost:3000/api/categories/debug | python3 -m json.tool || echo "Debug endpoint error"
echo ""

# Step 6: Clear browser cache reminder
echo ""
echo "======================================"
echo "Category Display Fix Applied!"
echo "======================================"
echo ""
echo "✅ FIXES APPLIED:"
echo "  - Categories API now returns correct format"
echo "  - Frontend expects 'category' field (not 'category_name')"
echo "  - Debug endpoint added at /api/categories/debug"
echo "  - Default categories ensured in database"
echo ""
echo "🔧 TO COMPLETE THE FIX:"
echo "  1. Clear your browser cache (Ctrl+Shift+Delete)"
echo "  2. Hard refresh the page (Ctrl+Shift+R)"
echo "  3. Go to http://15.204.252.189:3001"
echo "  4. Navigate to Items page"
echo ""
echo "Your 14 categories should now be visible!"
echo ""
echo "If still not showing, check the debug endpoint:"
echo "  curl http://15.204.252.189:3000/api/categories/debug"
echo ""
